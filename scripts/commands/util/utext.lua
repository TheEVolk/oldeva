local command = botcmd.mnew("ютекст", "генератор текста для обновлений", "<текст>", "d", {dev=1, right='photo'})
...
function command.exe(msg, args, other, rmsg, user, str)
    ...
    img:composite(
        magick.open_pseudo(1024, 100, "canvas:rgba(0, 0, 0, 0.5)"),
        200,
        (660 - 260) / 2
    )

    img:composite(
        magick.open_pseudo(1024, 200, "canvas:rgba(0, 0, 0, 0.5)"),
        200,
        (660 - 260) / 2
    )

    -- Icon
    local img_icon = magick.open(root.."/images/updsmile.png")
    img_icon:resize(300, 300)
    img:composite(img_icon, 50, 512 / 2 - 100)

    img:set_font_align(1)
    img:set_font(root.."/res/segoeuilight.ttf")

    img:set_font_size(60)
    img:annotate("rgb(255,255,255)", "Новое обновление!", 400, 265, 0)

    img:set_font_size(1200 / utf8.len(str))
    img:annotate("rgb(255,255,255)", str, 380, 360, 0)
